...
- 拉取 `ffinly/emby-collection-sync` 最新提交 `78039f2`。
- 同步核心榜单扩展：内置核心榜单从 10 个更新为 31 个，豆瓣分类榜单仍为 84 个。
- 新增 `notify_missing` 支持，允许榜单在通知中只展示缺失数量，避免长榜单刷屏。
- 同步旧合集名称清理映射：`Letterboxd's Top 500 Films` 会随 `LB Top 500 Films` 重建时自动清理。
